...
async function load() {
  const res = await fetch(`/api/data?media=${encodeURIComponent(media.id)}`);
...
async function run(raw) {
    ...
    for (const target of list) {
      try {
        ...
        if (target.id === media.id) {
          ...
          const res = await fetch(`/api/data?media=${encodeURIComponent(target.id)}`);
          const data = await res.json();
          count = (data.entries || []).length;
          const put = await fetch(`/api/data?media=${encodeURIComponent(target.id)}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
        ...
        print('err', `could not write to data/${target.id}.json — is the server running?`);
      }
    }
    renderAll();
    return;
  ...
  if (parsed.action.type === 'testmode') {
    try {
      const res = await fetch('/api/testmode', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ on: parsed.action.on })
      });
      if (!res.ok) throw new Error('testmode failed');
      const data = await res.json();
      await load(); // pick up whatever just changed in the active log's file
      renderAll();
      ...
      print('err', 'could not toggle test mode — is the server running?');
    }
    return;
  }
  ...
  if (changed) {
    try {
      ...
      print('err', `could not write to data/${media.id}.json — is the server running?`);
    }
  }
}
